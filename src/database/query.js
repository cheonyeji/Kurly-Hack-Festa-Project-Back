...
export const getDeliveryToDosQuery =
  "SELECT `delivery`.`tracking_num`, `delivery`.`receiver`, `delivery`.`address`, `delivery`.`user_request`, `user`.`phone_num` FROM `delivery` join `user` on `delivery`.`user_id` = `user`.`id`";

export const getDeliveryDonesQuery =
  "SELECT `delivery`.`tracking_num`, `delivery`.`receiver`, `delivery`.`address`, `delivery`.`user_request`, `user`.`phone_num`, `delivery`.`delivered_date` FROM `delivery` join `user` on `delivery`.`user_id` =  `user`.`id` where `delivery`.`delivered_date` is not null";
...
export const updateDeliveryToDoItemToStatusTwoQuery =
  insertIfNotExits + saveChatting + updateToStatusTwo;
export const updateDeliveryToDoItemToStatusThreeQuery =
  insertIfNotExits + saveChatting + updateToStatusThree;

// csController
export const getCSToDosQuery = "SELECT * FROM `cs` where `completed` = 0";
export const getCSDonesQuery = "SELECT * FROM `cs` where `completed` = 1`";
export const updateCSTodoItemQuery = "";
